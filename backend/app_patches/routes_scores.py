...
from app.services.storage import score_upload_dir, score_output_dir, shared_sheets_dir
...
async def upload_score(
    ...
    with Session(_get_engine()) as db:
        ...
        # Mirror upload to shared Sheets directory (best-effort)
        try:
            safe_orig = re.sub(r"[^\w.\-]", "_", file.filename)
            mirror = shared_sheets_dir() / f"{score.id}_{safe_orig}"
            mirror.write_bytes(content)
        except Exception as exc:
            logger.warning("Sheets mirror failed (non-fatal): %s", exc)
...
def list_scores(core_id: str = Depends(get_current_user)):
    with Session(_get_engine()) as db:
        statement = (
            select(Score)
            .where(Score.core_id == core_id)
...
def get_score(score_id: str, core_id: str = Depends(get_current_user)):
    _validate_score_id(score_id)
    with Session(_get_engine()) as db:
        score = db.get(Score, score_id)
        if not score or score.core_id != core_id:
...
def rename_score(score_id: str, body: ScoreRename, core_id: str = Depends(get_current_user)):
    _validate_score_id(score_id)
...
def delete_score(score_id: str, core_id: str = Depends(get_current_user)):
    _validate_score_id(score_id)
    with Session(_get_engine()) as db:
        score = db.get(Score, score_id)
        if not score or score.core_id != core_id:
    ...
    # Remove files from disk (Sheets/SheetsPDF mirrors are intentionally kept)
